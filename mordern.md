...
```
在 C++ 中，类的成员变量（尤其是非静态成员变量）的初始化时机，早于其所属类（宿主类）的构造函数体执行—— 这是 C++ 固定的对象构造顺序规则，即使你在宿主类的构造函数体中没有显式写初始化代码，编译器也会自动调用成员变量的默认构造函数完成初始化。